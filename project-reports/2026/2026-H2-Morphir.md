...
**Project Maintainers:** Attila Mihaly (Morgan Stanley), Damian Reeves (Capital One), Michelle Chan (Disney), Nwokafor Choongsaeng (Morgan Stanley), Peter Smulovics (Morgan Stanley), Stephen Goldbaum (DataHub)
...
**1. Contributor and organizational concentration — the primary risk.** A single contributor accounts for ~91% of human commits family-wide, and LFX reports 5 active contributors per quarter. The maintainer roster spans four organizations — Morgan Stanley, Capital One, Disney, and DataHub — and all six maintainers remain engaged in project direction, but day-to-day commit capacity is concentrated in far fewer hands than that suggests. This is the health metric the maintainers most want TOC help with, and the one where foundation-level reach makes more difference than maintainer effort alone.
...
**3. Issue and PR responsiveness — the clearest operational gap.** Approximately 326 open issues have gone untouched for more than 12 months, and `morphir-elm` — the repository with the highest external adoption — closed no issues in the reporting window. Much of that backlog is aged and superseded rather than live demand, and triage is prioritized in the Roadmap. The item that matters more is two external community pull requests ([#627](https://github.com/finos/morphir/pull/627), [#628](https://github.com/finos/morphir/pull/628)) open without review since March 2026; for a project working to rebuild its contributor funnel, responsiveness to outside contributions is the signal worth fixing first, and it is at the top of the remediation list.

**4. OSPS Baseline Level 3 — remediation underway.** Graduated status requires Maturity Level 3. No `SECURITY.md` is currently published in the repository family, and the project has no `project-data.yaml` (Security Insights v2.1.0) filed in the TOC repository. These are documentation artifacts rather than architectural gaps, and are scheduled in the Roadmap section.
...
**6. Community meetings — resumed, with record-keeping to restore.** Biweekly meeting agenda issues resumed on 2026-05-28 and have run on schedule since, following a hiatus from 2025-11-27. Recent agenda issues do not yet carry recorded attendance or approved minutes; restoring that practice is the first item on the remediation list.
...
**9. Release cadence and versioning.** The umbrella repository remains pre-1.0 (v0.3.3, January 2026) and `morphir-elm`'s last release was September 2025. `morphir-elm` is stable and in production use — its version number understates its maturity — but the maintainers recognize that the Graduated expectation of a production-ready major release is better served by version numbers that reflect reality, and will address this alongside the repository consolidation below.
...
Morphir occupies a position few open-source projects in financial services do: a working, production-deployed capability for making regulated business logic transparent and portable, with a reference implementation of a live Federal Reserve regulation behind it. As FINOS renews its push on regulatory digitization and the industry's attention turns to whether AI-generated logic can be trusted, that capability is a differentiated asset for the foundation as much as for the project. The requests below are framed accordingly — they are, for the most part, asks for reach and connection rather than for funding.
...
**8. Agreement on a remediation plan.** The maintainers intend to hold Graduated status and to close the gaps identified in this report rather than have them carried forward. What would be most useful from the TOC is agreement on the plan itself: confirmation that the priority ordering in the Roadmap addresses what the TOC considers material, and a clear statement of what evidence the TOC wants to see at the H1 2027 review. The maintainers would rather work to an agreed and realistic set of expectations than guess at them.

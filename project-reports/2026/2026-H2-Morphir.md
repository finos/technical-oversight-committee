...
**Project Maintainers:** Attila Mihaly (Morgan Stanley), Damian Reeves (Capital One), Michelle Chan (Disney), Nwokafor Choongsaeng (⟨TODO: organization⟩), Peter Smulovics (Morgan Stanley), Stephen Goldbaum (⟨TODO: organization⟩)
...
**This reporting period marks a deliberate strategic repositioning of the project**, described in Current Status below.
...
## Substrate — the new direction

**Substrate** ([PR #645](https://github.com/finos/morphir/pull/645), in active development since April 2026) is the project's answer, and is being presented to the TOC as Morphir's strategic direction for the next phase.

Substrate turns the documents an organization already writes — specifications, regulations, domain models, runbooks — into a structured, link-typed, **mechanically verifiable corpus** that both humans and LLMs can read, edit, and act on. Concretely:

- **The medium is plain GitHub-flavored markdown.** No custom syntax, renders natively on GitHub, written by humans with LLM assistance. Structure is carried by ordinary markdown features — headings, tables, lists, and especially **links between documents**.
- **Those links accumulate a semantic knowledge graph.** Every reference to a concept, type, or operation is a link to its definition. The corpus becomes traversable, type-checkable, and sliceable for LLM context — without constraining how authors write prose.
- **Verification is layered**: structural validity, link resolution, type compatibility, and embedded test cases that actually execute. Findings are reported against the originating prose, not against an opaque intermediate form.
- **The `substrate` CLI** ships context extraction (tree-shaken slices for LLMs), corpus validation, corpus-wide safe refactoring, embedded test execution, versioned package management, and live data binding.
- **Technology mappings** let engineers attach implementation patterns to recurring corpus structures, so matched patterns compile directly to TypeScript/SQL/etc. with no LLM improvisation — and novel cases handled by an LLM can be promoted into reusable patterns. Engineering judgment accumulates in the corpus instead of evaporating into chat transcripts.

This is continuous with Morphir's thesis, not a departure from it: the same insight that logic should be captured as inspectable, portable data, applied to the medium institutions actually use.
...
This directly extends the LCR work that made Morphir a candidate for FINOS regulatory digitization efforts several years ago, and it is the basis of a collaboration request in TOC Support Needed below.
...
**4. OSPS Baseline Level 3 — gaps acknowledged.** Graduated status requires Maturity Level 3. No `SECURITY.md` is currently published in any repository in the family, and the project has no `project-data.yaml` (Security Insights v2.1.0) filed in the TOC repository. A remediation plan with dates is in the Roadmap section.
...
**7. Repository sprawl.** The project spans 16 non-archived repositories, several of which are dormant (`morphir-jvm`: zero releases and 6 merged PRs in 12 months; `morphir-gleam`, `morphir-zig`, `morphir-moonbit`, `morphir-editor` at or near zero activity). Maintaining nominal ownership of dormant repositories dilutes attention and distorts health signals. A consolidation proposal is in the Roadmap.

**8. Release cadence.** The umbrella repository remains pre-1.0 (v0.3.3, January 2026) and `morphir-elm`'s last release was September 2025 — inconsistent with the Graduated expectation of a production-ready major release.

# Roadmap & Goals for Next 6 Months

## Compliance remediation (committed, with dates)

| Item | Target |
|---|---|
| Publish `SECURITY.md` across all active repositories | ⟨TODO: date⟩ |
| File `projects/morphir/project-data.yaml` (Security Insights v2.1.0) in the TOC repository | ⟨TODO: date⟩ |
| Complete OSPS Baseline Level 3 self-assessment and publish gap list | ⟨TODO: date⟩ |
| Publish `ROADMAP.md` in `finos/morphir` covering the Substrate direction | ⟨TODO: date⟩ |
| Complete `MAINTAINERS.md` (organizations for all entries) | ⟨TODO: date⟩ |
| Add root `CONTRIBUTING.md` to `finos/morphir` and fix the broken README link | ⟨TODO: date⟩ |
| Triage backlog: review or close the ~326 stale issues; review the two open external PRs | ⟨TODO: date⟩ |
| Resume recording meeting attendance and approving minutes | Immediate |
| Propose archival of dormant repositories to the TOC | ⟨TODO: date⟩ |

## Product and community goals

- **Land Substrate in `finos/morphir`.** Merge [PR #645](https://github.com/finos/morphir/pull/645) under Apache-2.0 (relicensing the Substrate subtree from its current MIT header to match FINOS IP policy), with the direction reflected in the public roadmap.
...
**5. Community Development Plan and go-to-market, built with FINOS staff.** Graduated status requires demonstrated progress against such a plan. The maintainers would like staff support to build one around the repositioned direction rather than the original 2020 framing.
